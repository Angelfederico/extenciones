...
import { normalizeMediaRedirectUrl, normalizeMediaThumbnailUrl } from "../core/contracts/api.js";
...
export function createGalleryController(deps) {
  ...
  async function openGalleryItemPreview(itemId) {
    ...
    try {
      openMediaPreview({
        ...
        index: activePreviewItems().findIndex((entry) => entry.id === itemId),
        url,
        kind: item.kind || "images",
        title: item.prompt || item.title || "Gallery preview"
      });
  ...
  function openMediaPreview({ items = [], index = 0, url, kind = "images", title = "Preview" } = {}) {
    ...
    let activeIndex = Math.max(0, Math.min(previewItems.length - 1, Number(index) || 0));
    if (url && activeIndex === -1) activeIndex = Math.max(0, previewItems.findIndex((item) => item.url === url));
    ...
    const renderPreview = () => {
      ...
      const media = isVideo
        ? node("video", { class: "gallery-preview-media", attrs: { src: item.url, controls: true, autoplay: true, muted: false, playsinline: true } })
