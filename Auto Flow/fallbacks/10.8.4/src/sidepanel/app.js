...
async function loadState() {
  ...
  state = mergeState(createDefaultState(), stored[STORAGE_KEY]);
...
function bindStorageUpdates() {
  ...
  chrome.storage.onChanged.addListener((changes, areaName) => {
    ...
    if (!appStateChange?.newValue && !queueLedgerChange?.newValue) return;
    const previousSignature = runtimeSurfaceSignature();
    if (appStateChange?.newValue) {
      state = mergeState(createDefaultState(), appStateChange.newValue);
    }
    const queueLedgerChanged = applyQueueLedgerStorageSnapshot(queueLedgerChange?.newValue);
...
function bindAfterControl(root) {
  ...
  root.querySelector("#refImageInput")?.addEventListener("change", (event) => {
    ...
    importReferenceFiles(files, {
      intent,
      activate: libraryOnly ? false : intent === "shared" || intent === "library" || intent === "match",
...
async function updatePreset(key, value, options = {}) {
  if (key === "videoLength" && state.control.mode === FLOW_MODES.ingredientsToVideo) {
    value = "8";
  }
...
function openReferenceUpload(root, forcedMode = "") {
  ...
  if (applyMode === "batch") {
    clickFileInputWithoutScroll(root.querySelector("#imageInput"));
    return;
  }
  state.control.activeApplyMode = applyMode;
  refImportIntent = applyMode === "match" ? "match" : "shared";
...
async function importReferenceFiles(fileList, options = {}) {
  ...
  state.control.activeApplyMode = importIntent === "match" ? "match" : "shared";
...
async function importOneToOneBatchReferences(input) {
  ...
  state.control.presets.mapLineRefs = true;
  state.control.promptMapOpen = true;
  appendLog("info", "refs", `1:1 batch imported ${added.length} image${added.length === 1 ? "" : "s"} and unchecked active library references${saveToLibrary ? "" : " for this run only"}.`);
  await persistState({ suppressStorageRender: true });
  render();
  restoreUiSnapshot(snapshot);
}
...
function promptMappedIds(mode, prompt, index, totalPrompts) {
  ...
  if (state.control.activeApplyMode === "match") {
    return matchedReferenceIdsForPrompt(prompt, savedItemsForIds(sourceIds), {
      limit: referenceLimitForMode(mode)
    });
  }
  const batchIds = oneToOneBatchRefIds();
  if (batchIds.length) {
...
function effectiveTextToImageModel(model = "nano_banana_pro", refInputs = [], options = {}) {
  return options.domFirst === true && Array.isArray(refInputs) && refInputs.length > 0
    ? "imagen_4"
...
async function buildJobs() {
  ...
  for (const [index, prompt] of prompts.entries()) {
    const promptPayload = promptPayloadForMode(mode, prompt);
    const mappedIds = promptMappedIds(mode, prompt, index, prompts.length).slice(0, referenceLimitForMode(mode));
...
function taskSettingsForMode(mode) {
  ...
  if (mode === FLOW_MODES.textToImage) {
    return {
      ...
      repeatCount: presets.imageRepeatCount || 1,
      download: {
  ...
  return {
    ...
    videoLength: mode === FLOW_MODES.ingredientsToVideo ? "8" : (presets.videoLength || "6"),
...
async function enqueueAndRun() {
  ...
  try {
    ...
    state.ui.activeRoute = "live";
    state.ui.galleryTab = state.control.mode === FLOW_MODES.textToImage ? "images" : "videos";
    showLocalPreparingQueue(prompts);
    appendLog("info", "queue", `Preparing ${prompts.length} prompt${prompts.length === 1 ? "" : "s"} for ${state.ui.galleryTab}.`);
    render();
...
async function resumeQueue() {
  ...
  applyRuntimePayload(response?.payload || {});
  if (Number(response?.payload?.resumed || 0) > 0) {
    state.ui.activeRoute = "live";
  }
  appendLog("info", "queue", `Resume requested for ${Number(response?.payload?.resumed || 0)} blocked task(s).`);
  await persistState();
  render();
}
