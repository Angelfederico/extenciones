...
import { buildRecoveryReportFields } from "../core/queue/recovery-policy.js";
...
const REVIEW_DUCK_DONE_STATUSES = new Set(["complete", "done"]);
const REVIEW_DUCK_SUPPORT_STATUSES = new Set(["failed", "blocked", "download_failed", "download_incomplete", "partial"]);
const REVIEW_DUCK_ACTIVE_STATUSES = new Set(["starting", "submitting", "generating", "downloading", "retrying", "pending", "queued", "waiting"]);
const REVIEW_DUCK_ACTION_KEYS = Object.freeze({
  success: ["chromeStore"],
  support: ["discord", "telegram", "exportReport"]
});
...
const INGREDIENTS_VIDEO_MODEL_OPTIONS = [
  ["omni_flash", "Omni Flash"],
  ["veo3_fast_low", "Veo 3.1 - Fast [Lower Priority]"],
  ["veo3_fast", "Veo 3.1 - Fast"]
];
...
export function renderControl(root, state, actions = {}) {
  ...
  const ingredientsMode = state.control.mode === FLOW_MODES.ingredientsToVideo;
  const omniT2v = state.control.mode === FLOW_MODES.textToVideo && /^(omni_flash|omni|abra)$/i.test(String(state.control.presets.model || ""));
  const omniI2v = state.control.mode === FLOW_MODES.imageToVideo && /^(omni_flash|omni|abra)$/i.test(String(state.control.presets.model || ""));
  const omniIngredients = ingredientsMode && /^(omni_flash|omni|abra)$/i.test(String(state.control.presets.ingredientsModel || ""));
  const activeVideoLength = ingredientsMode && !omniIngredients ? "8" : (state.control.presets.videoLength || "8");
  const durationChoices = (omniT2v || omniI2v || omniIngredients) ? ["4", "6", "8", "10"] : (ingredientsMode ? ["8"] : ["4", "6", "8"]);
  ...
  const inputSection = el("div", { id: "promptContainer", class: "control-section input-container" },
    ...
    el("div", { class: "prompt-input-toolbar" },
      el("div", { class: "prompt-input-title" },
        ...
        el("a", { class: "sample-link", attrs: { href: "/sample_prompt_formats.html", target: "_blank", rel: "noopener noreferrer", id: "samplePromptLink" }, text: "(Format Guide)" })
      ),
...
export function renderSettings(root, state) {
  ...
  const grid = el("div", { class: "settings-grid" },
    ...
    field("Submit Route:", controlGroup(
      el("div", { id: "settingsSubmitRouteButtonRow", class: "route-button-row" },
        routeBtn(presets.submitPath === "dom_first", "dom_first", "DOM"),
        routeBtn(presets.submitPath === "api_first", "api_first", "API")
      )
    )),
    el("div", { id: "videoModeSettings", class: "mode-settings" },
      ...
      field("Ingredients Video Model:", select("ingredientsModelSelector", presets.ingredientsModel, INGREDIENTS_VIDEO_MODEL_OPTIONS, "ingredientsModel")),
...
function renderLiveQueueGallery(state, selected = new Set(), options = {}) {
  ...
  return el("div", { id: "liveQueueContainer", class: "live-queue-wrap" },
    ...
    renderLiveQueueRecoveryCard(state, groups, locale),
    renderLiveQueueReviewDuck(groups, locale),
    groups.length
      ? groups.map((group) => el("div", { class: `live-queue-group is-${group.statusClass}` },
        renderLiveQueueStage(group, locale, state.queue?.runtimeEvents || []),
    ...
    renderHardQuotaModal(state, locale),
    renderModelAccessModal(state, locale),
    renderApiPathBlockedModal(state, locale),
    renderRepairFailedModal(state, locale)
  );
}

function renderLiveQueueRecoveryCard(state = {}, groups = [], locale = "en") {
  const items = visibleQueueItems(state.queue?.items || []);
  const runtimeEvents = Array.isArray(state.queue?.runtimeEvents) ? state.queue.runtimeEvents : [];
  const quotaTask = hardQuotaTask(items);
  if (quotaTask) {
    const fields = buildRecoveryReportFields(quotaTask, { tasks: items });
    return el("section", { class: "live-quota-banner", attrs: { role: "status" } },
      el("span", { class: "live-quota-icon" }, icon("warning")),
      el("span", { class: "live-quota-copy" },
        el("strong", { text: translate("flowQuotaReachedTitle", {}, locale) }),
        el("span", { text: translate("quotaCountsSummary", {
          completed: fields.completedTaskCountBeforeStop,
          downloaded: fields.downloadedCountBeforeStop,
          pending: fields.pendingTaskCountAfterStop
        }, locale) })
      ),
      el("button", { class: "live-quota-action", data: { copySupportSummary: quotaTask.id }, attrs: { type: "button", title: translate("copySupportSummary", {}, locale) } }, icon("content_copy"), el("span", { text: translate("copySupportSummary", {}, locale) }))
    );
  }
  const modelAccess = modelAccessTask(items);
  if (modelAccess) {
    const fields = buildRecoveryReportFields(modelAccess, { tasks: items });
    return el("section", { class: "live-model-access-banner", attrs: { role: "status" } },
      el("span", { class: "live-model-access-icon" }, icon("lock")),
      el("span", { class: "live-model-access-copy" },
        el("strong", { text: fields.userFacingFailureTitle || translate("flowModelAccessDeniedTitle", {}, locale) }),
        el("span", { text: translate("modelAccessSummary", {
          model: modelAccess.model || translate("unknown", {}, locale),
          mode: modeLabel(modelAccess.mode || "", locale),
          path: modelAccess.submitPath || modelAccess.submitPathPreference || translate("unknown", {}, locale)
        }, locale) })
      ),
      el("button", { class: "live-model-access-action", data: { modelAccessAction: "chooseSupportedModel", modelAccessTaskId: modelAccess.id }, attrs: { type: "button", title: translate("chooseSupportedModel", {}, locale) } }, icon("tune"), el("span", { text: translate("chooseSupportedModel", {}, locale) })),
      el("button", { class: "live-model-access-action", data: { copySupportSummary: modelAccess.id }, attrs: { type: "button", title: translate("copySupportSummary", {}, locale) } }, icon("content_copy"), el("span", { text: translate("copySupportSummary", {}, locale) }))
    );
  }
  const apiBlockedTask = apiPathBlockedTask(items);
  if (apiBlockedTask) {
    const fields = buildRecoveryReportFields(apiBlockedTask, { tasks: items });
    return el("section", { class: "live-api-path-banner", attrs: { role: "status" } },
      el("span", { class: "live-api-path-icon" }, icon("public")),
      el("span", { class: "live-api-path-copy" },
        el("strong", { text: translate("apiPathBlockedTitle", {}, locale) }),
        el("span", { text: translate("apiPathBlockedQuestion", {}, locale) }),
        el("small", { text: translate("apiPathBlockedSummary", {
          model: fields.modelSubmittedByApi || apiBlockedTask.model || translate("unknown", {}, locale),
          refs: fields.refCount,
          reused: fields.refsReusedForDomVerification ? translate("yes", {}, locale) : translate("no", {}, locale)
        }, locale) })
      ),
      el("button", { class: "live-api-path-action", data: { apiPathAction: "continueBrowserMode", apiPathTaskId: apiBlockedTask.id }, attrs: { type: "button", title: translate("continueWithBrowserMode", {}, locale) } }, icon("play_arrow"), el("span", { text: translate("continueWithBrowserMode", {}, locale) }))
    );
  }
  const latestRecovery = [...runtimeEvents].reverse().find((event) => String(event?.type || "").startsWith("flow_session_recovery."));
  const repairTask = items.find((item) => String(item?.recoveryPolicy || "") === "safe_recovery_ladder")
    || groups.flatMap((group) => group.items || []).find((item) => item?.flowSessionRecoveryAttempts);
  if (!latestRecovery && !repairTask) return null;
  const recovered = latestRecovery?.type === "flow_session_recovery.success" || repairTask?.recoveryFinalOutcome === "recovered";
  const failed = latestRecovery?.type === "flow_session_recovery.pause_unresolved" || repairTask?.recoveryFinalOutcome === "repair_failed";
  const task = repairTask || {};
  const step = task.recoveryCurrentStep || latestRecovery?.recoveryLevel || "classify_and_snapshot";
  const attempt = Number(task.recoveryAttemptCount || latestRecovery?.attempt || task.flowSessionRecoveryAttempts || 1) || 1;
  const failureClass = task.failureClass || latestRecovery?.failureClass || "flow_session_heat";
  return el("section", { class: `live-recovery-card${recovered ? " is-recovered" : ""}${failed ? " is-failed" : ""}`, attrs: { role: "status" } },
    el("span", { class: "live-recovery-spinner" }, icon(recovered ? "check_circle" : (failed ? "error" : "sync"))),
    el("span", { class: "live-recovery-copy" },
      el("strong", { text: translate(recovered ? "flowRecovered" : (failed ? "flowRepairFailed" : "repairingFlowSession"), {}, locale) }),
      el("span", { text: translate(recovered ? "flowRecoveredBody" : (failed ? "flowRepairFailedBody" : "repairingFlowSessionBody"), {}, locale) }),
      el("small", { text: translate("recoveryStepSummary", {
        step,
        attempt,
        failureclass: failureClass,
        mode: modeLabel(task.mode || "", locale),
        modelsuffix: task.model ? ` - ${task.model}` : ""
      }, locale) })
    )
  );
}

function hardQuotaTask(items = []) {
  return (items || []).find((item) => item?.recoverySkippedBecauseHardQuota === true || item?.failureClass === "flow_model_daily_quota_reached") || null;
}

function modelAccessTask(items = []) {
  return (items || []).find((item) => item?.recoverySkippedBecauseModelAccess === true ||
    item?.failureClass === "flow_model_access_denied" ||
    item?.failureClass === "api_first_model_access_denied") || null;
}

function apiPathBlockedTask(items = []) {
  return (items || []).find((item) => item?.apiFirstQuotaSuspected === true &&
    item?.domVerificationAttempted === true &&
    (
      item?.domVerificationResult === "success" ||
      item?.finalQuotaClassification === "api_first_blocked_dom_available" ||
      item?.failureClass === "api_first_blocked_dom_available"
    )) || null;
}

function repairFailedTask(items = []) {
  return (items || []).find((item) => item?.recoveryFinalOutcome === "repair_failed" ||
    item?.failureClass === "flow_session_heat_unresolved") || null;
}

function renderHardQuotaModal(state = {}, locale = "en") {
  const items = visibleQueueItems(state.queue?.items || []);
  const task = hardQuotaTask(items);
  if (!task) return null;
  const fields = buildRecoveryReportFields(task, { tasks: items });
  return el("div", { class: "live-quota-modal-backdrop", attrs: { role: "dialog", "aria-modal": "true", "aria-label": translate("flowQuotaReachedTitle", {}, locale) } },
    el("section", { class: "live-quota-modal" },
      el("button", { class: "live-quota-close", data: { quotaAction: "resumeLater" }, attrs: { type: "button", title: translate("resumeLater", {}, locale) } }, icon("close")),
      el("span", { class: "live-quota-modal-icon" }, icon("warning")),
      el("h3", { text: translate("flowQuotaReachedTitle", {}, locale) }),
      el("p", { text: translate("flowQuotaReachedBody", {}, locale) }),
      el("dl", { class: "live-quota-detail-grid" },
        quotaDetail("deployed_code", translate("model", {}, locale), task.model || translate("unknown", {}, locale)),
        quotaDetail("movie", translate("mode", {}, locale), modeLabel(task.mode || "", locale)),
        quotaDetail("check_circle", translate("completedBeforeStop", {}, locale), String(fields.completedTaskCountBeforeStop)),
        quotaDetail("download", translate("downloadedBeforeStop", {}, locale), String(fields.downloadedCountBeforeStop)),
        quotaDetail("schedule", translate("pendingCount", {}, locale), String(fields.pendingTaskCountAfterStop)),
        quotaDetail("warning", translate("error", {}, locale), task.lastError || task.flowErrorCode || "429 RESOURCE_EXHAUSTED", "is-error")
      ),
      el("div", { class: "live-quota-next" },
        el("strong", { text: translate("whatYouCanDo", {}, locale) }),
        el("ul", null,
          el("li", { text: translate("waitForQuotaReset", {}, locale) }),
          el("li", { text: translate("switchToAnotherModel", {}, locale) }),
          el("li", { text: translate("reduceRepeatResumeLater", {}, locale) })
        )
      ),
      el("div", { class: "live-quota-modal-actions" },
        el("button", { class: "primary", data: { quotaAction: "resumeLater" }, attrs: { type: "button" } }, icon("schedule"), el("span", { text: translate("resumeLater", {}, locale) })),
        el("button", { data: { quotaAction: "chooseSupportedModel" }, attrs: { type: "button" } }, icon("tune"), el("span", { text: translate("changeModelForPendingRows", {}, locale) })),
        el("button", { data: { copySupportSummary: task.id }, attrs: { type: "button" } }, icon("content_copy"), el("span", { text: translate("copySupportSummary", {}, locale) })),
        el("button", { data: { quotaAction: "exportReport" }, attrs: { type: "button" } }, icon("download"), el("span", { text: translate("exportReport", {}, locale) }))
      )
    )
  );
}

function renderModelAccessModal(state = {}, locale = "en") {
  const items = visibleQueueItems(state.queue?.items || []);
  const task = modelAccessTask(items);
  if (!task) return null;
  const fields = buildRecoveryReportFields(task, { tasks: items });
  const affected = items.filter((item) => item?.failureClass === task.failureClass || item?.model === task.model).length || 1;
  return el("div", { class: "live-model-access-modal-backdrop", attrs: { role: "dialog", "aria-modal": "true", "aria-label": translate("flowModelAccessDeniedTitle", {}, locale) } },
    el("section", { class: "live-model-access-modal" },
      el("button", { class: "live-model-access-close", data: { modelAccessAction: "pause" }, attrs: { type: "button", title: translate("pause", {}, locale) } }, icon("close")),
      el("span", { class: "live-model-access-modal-icon" }, icon("lock")),
      el("h3", { text: translate("flowModelAccessDeniedTitle", {}, locale) }),
      el("p", { text: fields.userFacingFailureBody || translate("flowModelAccessDeniedBody", {}, locale) }),
      el("dl", { class: "live-quota-detail-grid" },
        quotaDetail("deployed_code", translate("model", {}, locale), task.model || translate("unknown", {}, locale)),
        quotaDetail("movie", translate("mode", {}, locale), modeLabel(task.mode || "", locale)),
        quotaDetail("route", translate("submitPathLabel", {}, locale), task.submitPath || task.submitPathPreference || translate("unknown", {}, locale)),
        quotaDetail("format_list_numbered", translate("affectedTaskCount", {}, locale), String(affected)),
        quotaDetail("warning", translate("error", {}, locale), task.lastError || task.flowErrorCode || "403 PERMISSION_DENIED", "is-error")
      ),
      el("div", { class: "live-quota-modal-actions" },
        el("button", { class: "primary", data: { modelAccessAction: "chooseSupportedModel", modelAccessTaskId: task.id }, attrs: { type: "button" } }, icon("tune"), el("span", { text: translate("chooseSupportedModel", {}, locale) })),
        el("button", { data: { modelAccessAction: "switchPendingRowsToSelectedModel", modelAccessTaskId: task.id }, attrs: { type: "button" } }, icon("swap_horiz"), el("span", { text: translate("switchPendingRowsToSelectedModel", {}, locale) })),
        el("button", { data: { copySupportSummary: task.id }, attrs: { type: "button" } }, icon("content_copy"), el("span", { text: translate("copySupportSummary", {}, locale) })),
        el("button", { data: { modelAccessAction: "exportReport" }, attrs: { type: "button" } }, icon("download"), el("span", { text: translate("exportReport", {}, locale) }))
      )
    )
  );
}

function renderApiPathBlockedModal(state = {}, locale = "en") {
  const items = visibleQueueItems(state.queue?.items || []);
  const task = apiPathBlockedTask(items);
  if (!task) return null;
  const fields = buildRecoveryReportFields(task, { tasks: items });
  return el("div", { class: "live-api-path-modal-backdrop", attrs: { role: "dialog", "aria-modal": "true", "aria-label": translate("apiPathBlockedTitle", {}, locale) } },
    el("section", { class: "live-api-path-modal" },
      el("button", { class: "live-api-path-close", data: { apiPathAction: "pause" }, attrs: { type: "button", title: translate("pause", {}, locale) } }, icon("close")),
      el("span", { class: "live-api-path-modal-icon" }, icon("public")),
      el("h3", { text: translate("apiPathBlockedTitle", {}, locale) }),
      el("p", { text: translate("apiPathBlockedBody", {}, locale) }),
      el("dl", { class: "live-quota-detail-grid" },
        quotaDetail("deployed_code", translate("model", {}, locale), fields.modelSubmittedByApi || task.model || translate("unknown", {}, locale)),
        quotaDetail("movie", translate("mode", {}, locale), modeLabel(task.mode || "", locale)),
        quotaDetail("image", translate("referenceCount", {}, locale), String(fields.refCount)),
        quotaDetail("sync_saved_locally", translate("refsReused", {}, locale), fields.refsReusedForDomVerification ? translate("yes", {}, locale) : translate("no", {}, locale)),
        quotaDetail("playlist_add_check", translate("pendingRowsPreserved", {}, locale), fields.pendingRowsPreserved ? translate("yes", {}, locale) : translate("no", {}, locale))
      ),
      el("div", { class: "live-quota-modal-actions" },
        el("button", { class: "primary", data: { apiPathAction: "continueBrowserMode", apiPathTaskId: task.id }, attrs: { type: "button" } }, icon("play_arrow"), el("span", { text: translate("continueWithBrowserMode", {}, locale) })),
        el("button", { data: { apiPathAction: "pause" }, attrs: { type: "button" } }, icon("pause"), el("span", { text: translate("pause", {}, locale) })),
        el("button", { data: { copySupportSummary: task.id }, attrs: { type: "button" } }, icon("content_copy"), el("span", { text: translate("copySupportSummary", {}, locale) })),
        el("button", { data: { apiPathAction: "exportReport" }, attrs: { type: "button" } }, icon("download"), el("span", { text: translate("exportReport", {}, locale) }))
      )
    )
  );
}

function renderRepairFailedModal(state = {}, locale = "en") {
  const items = visibleQueueItems(state.queue?.items || []);
  const task = repairFailedTask(items);
  if (!task) return null;
  const fields = buildRecoveryReportFields(task, { tasks: items });
  const steps = Array.isArray(fields.recoveryStepsAttempted) && fields.recoveryStepsAttempted.length
    ? fields.recoveryStepsAttempted.join(", ")
    : (task.recoveryCurrentStep || "safe recovery ladder");
  return el("div", { class: "live-repair-failed-modal-backdrop", attrs: { role: "dialog", "aria-modal": "true", "aria-label": translate("repairFailedModalTitle", {}, locale) } },
    el("section", { class: "live-repair-failed-modal" },
      el("button", { class: "live-repair-failed-close", data: { repairAction: "pause" }, attrs: { type: "button", title: translate("pause", {}, locale) } }, icon("close")),
      el("span", { class: "live-repair-failed-modal-icon" }, icon("build_circle")),
      el("h3", { text: translate("repairFailedModalTitle", {}, locale) }),
      el("p", { text: translate("repairFailedModalBody", {}, locale) }),
      el("dl", { class: "live-quota-detail-grid" },
        quotaDetail("build", translate("recoveryStepsAttempted", {}, locale), steps),
        quotaDetail("error", translate("finalError", {}, locale), task.lastError || task.flowErrorCode || task.failureClass || translate("unknown", {}, locale), "is-error"),
        quotaDetail("recommend", translate("recommendedAction", {}, locale), fields.recommendedNextAction || translate("resumeLater", {}, locale))
      ),
      el("div", { class: "live-quota-modal-actions" },
        el("button", { class: "primary", data: { repairAction: "pause" }, attrs: { type: "button" } }, icon("pause"), el("span", { text: translate("pause", {}, locale) })),
        el("button", { data: { copySupportSummary: task.id }, attrs: { type: "button" } }, icon("content_copy"), el("span", { text: translate("copySupportSummary", {}, locale) })),
        el("button", { data: { repairAction: "exportReport" }, attrs: { type: "button" } }, icon("download"), el("span", { text: translate("exportReport", {}, locale) }))
      )
    )
  );
}

function quotaDetail(iconName, label, value, className = "") {
  return el("div", { class: `live-quota-detail${className ? ` ${className}` : ""}` },
    el("dt", null, icon(iconName), el("span", { text: label })),
    el("dd", { text: value || "" })
  );
}

function renderLiveQueueReviewDuck(groups = [], locale = "en") {
  const items = groups.flatMap((group) => Array.isArray(group.items) ? group.items : []);
  if (!items.length) return null;
  const statuses = items.map((item) => liveQueueEffectiveStatus(item, { queueRunning: false }));
  const hasActive = statuses.some((status) => REVIEW_DUCK_ACTIVE_STATUSES.has(status));
  if (hasActive) return null;
  const hasSupport = statuses.some((status) => REVIEW_DUCK_SUPPORT_STATUSES.has(status));
  const allComplete = statuses.every((status) => REVIEW_DUCK_DONE_STATUSES.has(status));
  if (!hasSupport && !allComplete) return null;

  const kind = hasSupport ? "support" : "success";
  const actionKeys = REVIEW_DUCK_ACTION_KEYS[kind] || [];
  const ariaKey = kind === "support" ? "reviewDuckSupportAria" : "reviewDuckSuccessAria";
  const statusKey = kind === "support" ? "reviewDuckSupportStatus" : "reviewDuckCompleteStatus";
  const copyKey = kind === "support" ? "reviewDuckSupportCopy" : "reviewDuckSuccessCopy";
  return el("aside", {
    class: `review-duck-banner is-${kind}`,
    attrs: {
      "aria-label": translate(ariaKey, {}, locale),
      "data-review-duck-state": kind
    }
  },
    el("span", { class: "review-duck-stage", attrs: { "aria-hidden": "true" } },
      el("span", { class: "review-duck-sprite" }),
      kind === "success" ? el("span", { class: "review-duck-confetti one" }) : null,
      kind === "success" ? el("span", { class: "review-duck-confetti two" }) : null,
      kind === "success" ? el("span", { class: "review-duck-confetti three" }) : null
    ),
    el("span", { class: "review-duck-bubble" },
      el("span", { class: "review-duck-status", text: translate(statusKey, {}, locale) }),
      el("span", { class: "review-duck-message", text: translate(copyKey, {}, locale) }),
      el("span", { class: "review-duck-actions" },
        actionKeys.map((action) => renderReviewDuckAction(action, locale))
      )
    )
  );
}

function renderReviewDuckAction(action, locale = "en") {
  const labels = {
    chromeStore: "reviewDuckChromeStore",
    discord: "reviewDuckDiscord",
    telegram: "reviewDuckTelegram",
    exportReport: "reviewDuckExportReport"
  };
  const labelKey = labels[action] || "reviewDuckChromeStore";
  return el("button", {
    class: "review-duck-link",
    data: { reviewDuckAction: action },
    attrs: { type: "button", title: translate(labelKey, {}, locale) }
  }, el("span", { text: translate(labelKey, {}, locale) }));
}
...
function renderLiveQueueStage(group = {}, locale = "en", runtimeEvents = []) {
  const first = group.items?.[0] || {};
  ...
  const apiBackendDomFallback = isApi && groupHasApiBackendDomFallback(group, runtimeEvents);
  const backendActive = isApi && !apiBackendDomFallback;
  const progress = liveQueueGroupProgress(group.items || []);
  const stage = liveQueueStageMeta(activeStatus, backendActive, progress, locale);
  const percent = progress.expectedCount ? Math.min(100, Math.round((progress.resultCount / progress.expectedCount) * 100)) : 0;
  const pathIcon = apiBackendDomFallback ? "sync" : (backendActive ? "cloud_sync" : "ads_click");
  const pathLabel = apiBackendDomFallback
    ? translate("apiToDomFallbackPath", {}, locale)
    : (backendActive ? translate("apiPath", {}, locale) : translate("domPath", {}, locale));
  const pathDetail = apiBackendDomFallback
    ? translate("apiToDomFallbackRun", {}, locale)
    : (repairActive ? translate("browserRunWithRepair", {}, locale) : (backendActive ? translate("backendRun", {}, locale) : translate("browserRun", {}, locale)));
  return el("div", { class: `live-queue-stage is-${activeStatus}` },
    ...
    el("div", { class: "live-stage-copy" },
      ...
      el("span", { class: `live-stage-mode-pill ${backendActive ? "is-api" : "is-dom"}${apiBackendDomFallback ? " is-fallback" : ""}` },
        icon(pathIcon),
        el("b", { text: pathLabel }),
        el("em", { text: pathDetail })
      ),
...
function groupHasApiBackendDomFallback(group = {}, runtimeEvents = []) {
  const taskIds = new Set((group.items || [])
    .flatMap((item) => [item?.id, item?.taskId])
    .map((id) => String(id || "").trim())
    .filter(Boolean));
  const taskMatches = (event = {}) => {
    if (!taskIds.size) return true;
    return taskIds.has(String(event.taskId || "").trim());
  };
  if ((group.items || []).some((item) => String(item?.submitPath || "") === "dom_fallback" || item?.repairedFromApi === true)) {
    return true;
  }
  return (runtimeEvents || []).some((event) => {
    if (!taskMatches(event)) return false;
    const type = String(event?.type || "");
    if (type === "queue.api_backend.dom_fallback" || type === "queue.api_backend.dom_fallback_failed") return true;
    return type === "queue.submit.ok"
      && String(event?.submitPath || "") === "api_first"
      && String(event?.path || "") === "dom"
      && event?.repairedFromApi === true;
  });
}
...
function isT2iSavedReferenceAssetRowMissing(item = {}) {
  return item.failureClass === "t2i_saved_reference_asset_row_missing";
}
...
function renderT2iSavedReferenceRepairCard(item = {}, locale = "en") {
  if (!isT2iSavedReferenceAssetRowMissing(item)) return null;
  const taskId = String(item.id || "");
  const fileName = item.userActionRequiredReferenceFileName || item.refInputs?.[0]?.fileName || translate("t2iSavedReferenceDefaultFileName", {}, locale);
  const defaultApiError = translate("t2iSavedReferenceDefaultApiError", {}, locale);
  const originalError = item.apiBackendError || (/500/i.test(String(item.lastError || "")) ? defaultApiError : item.lastError || defaultApiError);
  const recovery = item.finalFallbackOutcome || translate("t2iSavedReferenceDefaultRecovery", {}, locale);
  return el("div", { class: "live-task-reference-repair" },
    el("div", { class: "live-task-reference-repair-copy" },
      el("strong", { text: item.userFacingFailureTitle || translate("t2iSavedReferenceMissingTitle", {}, locale) }),
      el("span", { text: item.userFacingFailureBody || translate("t2iSavedReferenceDefaultBody", {}, locale) }),
      el("dl", { class: "live-task-reference-repair-details" },
        el("dt", { text: translate("t2iSavedReferenceFileLabel", {}, locale) }), el("dd", { text: fileName }),
        el("dt", { text: translate("mode", {}, locale) }), el("dd", { text: translate("createImage", {}, locale) }),
        el("dt", { text: translate("model", {}, locale) }), el("dd", { text: modelDisplayName(item.model) }),
        el("dt", { text: translate("t2iSavedReferenceOriginalErrorLabel", {}, locale) }), el("dd", { text: originalError }),
        el("dt", { text: translate("t2iSavedReferenceRecoveryLabel", {}, locale) }), el("dd", { text: recovery })
      )
    ),
    el("div", { class: "live-task-reference-repair-actions" },
      el("button", { data: { liveTaskAction: "reuploadReference", taskId }, attrs: { type: "button" } }, icon("upload"), el("span", { text: translate("t2iSavedReferenceReuploadAction", {}, locale) })),
      el("button", { data: { liveTaskAction: "retryBrowserMode", taskId }, attrs: { type: "button" } }, icon("restart_alt"), el("span", { text: translate("t2iSavedReferenceRetryBrowserAction", {}, locale) })),
      el("button", { data: { liveTaskAction: "exportReport", taskId }, attrs: { type: "button" } }, icon("download"), el("span", { text: translate("reviewDuckExportReport", {}, locale) })),
      el("button", { data: { liveTaskAction: "copySupportSummary", taskId }, attrs: { type: "button" } }, icon("content_copy"), el("span", { text: translate("t2iSavedReferenceCopySupportAction", {}, locale) }))
    )
  );
}
...
function renderLiveQueueTaskRow(item = {}, index = 0, selected = new Set(), options = {}, locale = "en") {
  ...
  const label = liveQueueStatusLabel(item, options, locale);
  ...
  return el("div", { class: `live-queue-task is-${groupStatusClass([item], options)}` },
    ...
    isErrorState
      ? el("button", { class: "live-task-support-btn", data: { copySupportSummary: item.id }, attrs: { type: "button", title: translate("copySupportSummary", {}, locale) } }, icon("content_copy"), el("span", { text: translate("copySupportSummary", {}, locale) }))
      : el("span", { class: "live-task-support-spacer" }),
    ...
    renderT2iSavedReferenceRepairCard(item, locale)
  );
}
...
function liveQueueProgress(item = {}) {
  ...
  const isVideo = liveQueueIsVideoItem(item);
  if (isVideo) {
    const successfulStatusRows = Array.isArray(item.statusRows)
      ? item.statusRows.filter((row) => String(row?.status || "").toLowerCase() === "complete")
      : [];
    const resultCount = Number(item.foundVideos || ledger.resultCount || successfulStatusRows.length || 0);
...
function liveQueueStatusLabel(item = {}, options = {}, locale = "en") {
  const status = liveQueueEffectiveStatus(item, options);
  ...
  if (status === "failed") {
    if (item.failureClass === "t2i_saved_reference_asset_row_missing") return translate("t2iSavedReferenceMissingTitle", {}, locale);
