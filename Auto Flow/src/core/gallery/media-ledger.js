...
function isPortraitVideoAspect(value = "") {
  const normalized = String(value || "").trim().toLowerCase();
  return normalized === "portrait"
    || normalized === "9:16"
    || normalized === "video_aspect_ratio_portrait";
}
...
function downloadResolutionForItem(item = {}, options = {}) {
  ...
  if (kind === "videos") {
    const resolution = normalizeVideoDownloadResolution(taskResolution || options.videoResolution || options.resolution || item.videoResolution || itemResolution || "720p");
    return isPortraitVideoAspect(item.aspectRatio) && resolution !== "720p" ? "720p" : resolution;
  }
  return "";
}
...
export function canonicalGalleryItems(items = [], options = {}) {
  ...
  for (const [index, item] of filterUsableGalleryItems(scopedItems, {
    ...
    if (!key) continue;
    const selectedHit = selectedMatchesGalleryItem(selected, item);
  ...
  for (const task of tasks || []) {
    ...
    for (let mediaIndex = 0; mediaIndex < placeholderCount; mediaIndex += 1) {
      ...
      const item = {
        ...
        aspectRatio: String(output.aspectRatio || task.aspectRatio || ""),
        referenceIds,
...
export function reconcileTasksWithDownloadResults(tasks = [], downloads = [], now = new Date().toISOString()) {
  ...
  for (const task of tasks || []) {
    ...
    const downloadErrorMediaIds = [...new Set([
      ...
      ...relevant
        .filter((download) => !download.ok && !download.skipped)
        .map((download) => String(download.mediaId || "").trim())
        .filter(Boolean)
    ...
    patches.push({
      ...
      patch: {
        ...
        ...foundPatch,
        lastDownloadAt: now,
        lastDownloadError: downloadErrorMediaIds.length ? (task.lastDownloadError || "") : ""
      },
      downloadedCount: downloadedMediaIds.length,
      skippedDownloadCount: skippedDownloadMediaIds.length
    });
...
function filenameWithNumericSuffix(filename = "", suffix = 2) {
  const text = String(filename || "");
  const safeSuffix = Math.max(2, Number.parseInt(suffix, 10) || 2);
  const slashIndex = Math.max(text.lastIndexOf("/"), text.lastIndexOf("\\"));
  const dir = slashIndex >= 0 ? text.slice(0, slashIndex + 1) : "";
  const leaf = slashIndex >= 0 ? text.slice(slashIndex + 1) : text;
  const dotIndex = leaf.lastIndexOf(".");
  if (dotIndex > 0) {
    return `${dir}${leaf.slice(0, dotIndex)}-${safeSuffix}${leaf.slice(dotIndex)}`;
  }
  return `${dir}${leaf}-${safeSuffix}`;
}

function reserveTargetFilename(filename = "", seenTargets = new Set(), options = {}) {
  let targetPathKey = normalizeDownloadTargetPathKey(filename);
  if (!seenTargets.has(targetPathKey)) {
    return { filename, targetPathKey, duplicateTargetSuffix: 0 };
  }
  if (options.allowDuplicateTargetSuffix !== true) return null;
  for (let suffix = 2; suffix <= 999; suffix += 1) {
    const candidate = filenameWithNumericSuffix(filename, suffix);
    targetPathKey = normalizeDownloadTargetPathKey(candidate);
    if (!seenTargets.has(targetPathKey)) {
      return { filename: candidate, targetPathKey, duplicateTargetSuffix: suffix };
    }
  }
  return null;
}

function selectedMatchesGalleryItem(selected = new Set(), item = {}) {
  ...
  const candidates = [
    ...
    item.name,
    item.mediaName,
    item.taskId && item.mediaId ? `${item.taskId}:${item.mediaId}` : ""
...
export function planMediaDownloads(items = [], options = {}) {
  ...
  for (const { item, index } of downloadItems) {
    ...
    const targetReservation = reserveTargetFilename(filename, seenTargets, {
      allowDuplicateTargetSuffix: options.allowDuplicateTargetSuffix === true
    });
    if (!targetReservation) continue;
    const plannedFilename = targetReservation.filename;
    const targetPathKey = targetReservation.targetPathKey;
    ...
    plans.push({
      ...
      dedupeDecision: targetReservation.duplicateTargetSuffix ? "allowed_unique_target" : "allowed"
    });
  }
  return plans;
}
