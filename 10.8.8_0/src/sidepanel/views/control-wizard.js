...
function modelKeyForMode(mode = "") {
  if (mode === FLOW_MODES.textToImage) return "imageModel";
  if (mode === FLOW_MODES.ingredientsToVideo) return "ingredientsModel";
  return "model";
}

function modelOptionsForMode(mode = "") {
  if (mode === FLOW_MODES.textToImage) {
    return ["nano_banana_pro", "nano_banana_2", "imagen_4"].map((key) => [key, formatModelLabel(key)]);
  }
  if (mode === FLOW_MODES.ingredientsToVideo) return INGREDIENTS_VIDEO_MODEL_OPTIONS;
  if (mode === FLOW_MODES.textToVideo) return TEXT_VIDEO_MODEL_OPTIONS;
...
function runBlockerLabel(state, promptCount = 0) {
  ...
  if (mode?.needsRefs) {
    ...
    if (!refIds.length && !hasNativeCharacterRefs) return tr(state, "addRefToRun");
  }
  return "";
}
...
function buildAccountStrip(state) {
  ...
  const usageLabel = info.signedIn
    ...
    : translate("signInRequired", {}, locale);
...
async function handleModeChange(newMode, state, dispatch, showConfirm) {
  ...
  if (!hasPrompts && !hasRefs) {
    dispatch({ control: { mode: newMode, activeApplyMode: "shared" } });
    return;
  }
  if (!showConfirm) {
    dispatch({ control: { mode: newMode, activeApplyMode: "shared" } });
...
function buildStep1Body(state, text, prompts, dispatch) {
  const activeTab = state.control?.promptTab === "character_prompts" ? "character_prompts" : "workflow_prompts";
  const mode = state.control?.mode || FLOW_MODES.textToVideo;
  const presets = state.control?.presets || {};
  const modelKey = modelKeyForMode(mode);
  const modelOptions = modelOptionsForMode(mode);
  const characterText = characterPromptText(state);
  const characterAnalysis = characterRunAnalysis(state, prompts);
  const modelRow = el("div", { class: "afw-model-row" },
    el("div", { class: "afw-model-row-label" },
      el("span", { class: "material-symbols-outlined", text: "tune" }),
      el("span", { text: tr(state, "generationModel") }),
      el("small", { text: tr(state, "modelOverrideStep1") })
    ),
    compactSelect(modelKey, presets[modelKey], modelOptions)
  );
  const tabs = el("div", { class: "afw-tabs", attrs: { role: "tablist" } },
    el("button", {
  ...
  if (activeTab === "character_prompts") {
    return buildCharacterPromptStepBody(state, characterText, characterAnalysis, dispatch, modelRow, tabs);
  }
  let previewWrap;
  let countNode;
  ...
  return el("div", { class: "afw-step-body" },
    ...
    el("div", { class: "afw-paste" },
      ...
      el("div", { class: "afw-paste-row" },
        ...
        el("button", { id: "pasteAutoFlowPacketButton", class: "afw-back", attrs: { type: "button", style: "padding:5px 10px;font-size:10.5px;" } }, icon("assignment"), tr(state, "pasteAutoFlowPacket")),
        ct,
      ),
    ),
...
function buildCharacterPromptStepBody(state, text, analysis, dispatch, modelRow, tabs) {
  let listWrap;
  let countNode;
  let issueNode;
  ...
  return el("div", { class: "afw-step-body" },
    ...
    el("div", { class: "afw-paste" },
      ...
      el("div", { class: "afw-paste-row" },
        ...
        el("button", { id: "pasteAutoFlowPacketButtonCharacter", class: "afw-back", attrs: { type: "button", style: "padding:5px 10px;font-size:10.5px;" } }, icon("assignment"), tr(state, "pasteAutoFlowPacket")),
...
function buildStep2Body(state, dispatch) {
  ...
  const activeTab = state.control?.referencesTab === "character_sources" ? "character_sources" : "prompt_references";
  const tabs = buildSegmentTabs(state, [
...
function buildStep3Body(state, prompts, refs, mode, dispatch = () => {}) {
  ...
  const settingsCard = el("div", { class: "afw-gen-card" },
    ...
    mode.key !== FLOW_MODES.textToImage && (mode.key !== FLOW_MODES.ingredientsToVideo || durationOptions.length > 1)
