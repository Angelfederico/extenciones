...
export function isOmniVideoModel(value = "") {
  return /^(omni_flash|omni|abra)$/i.test(String(value || "").trim());
}

const FULL_FRAME_CROP_COORDINATES = Object.freeze({
  top: 0,
  left: 0,
  bottom: 1,
  right: 1
...
export function normalizeCropCoordinates(value = FULL_FRAME_CROP_COORDINATES) {
  const source = value && typeof value === "object" ? value : FULL_FRAME_CROP_COORDINATES;
  ...
  if (![top, left, bottom, right].every(Number.isFinite)) {
    return { ...FULL_FRAME_CROP_COORDINATES };
  }
...
export function normalizeVideoModelKey(mode, value = "default", aspectRatio = "landscape", options = {}) {
  ...
  let raw = incoming === "veo_3_1" || incoming === "veo3" || incoming === "veo-3-1"
    ? "default"
    : incoming;
  ...
  if (mode !== "t2v" && mode !== "i2v" && mode !== "r2v" && isOmniVideoModel(raw)) raw = "default";
  ...
  if (mode === "t2v") {
    if (isOmniVideoModel(raw)) {
      return `abra_t2v_${duration}s`;
    }
  ...
  if (mode === "i2v") {
    ...
    if (hasEndImage) return isPortrait ? "veo_3_1_i2v_s_fast_portrait_fl" : "veo_3_1_i2v_s_fast_fl";
    return isPortrait ? "veo_3_1_i2v_s_fast_portrait_ultra" : "veo_3_1_i2v_s_fast_ultra";
  }

  if (mode === "r2v") {
    if (isOmniVideoModel(raw)) {
      return `abra_r2v_${duration}s`;
    }
...
export function buildVideoStartImageBody({
  ...
  return {
    ...
    requests: Array.from({ length: requestCount }, () => ({
      ...
      metadata: {},
      startImage: {
        mediaId: String(startMediaId || "").trim(),
        cropCoordinates: normalizeCropCoordinates(startCropCoordinates)
      }
    })),
    useV2ModelConfig: true
  };
...
export function buildVideoStartAndEndImageBody({
  ...
  return {
    ...
    requests: Array.from({ length: requestCount }, () => ({
      ...
      endImage: {
        mediaId: String(endMediaId || "").trim(),
        cropCoordinates: normalizeCropCoordinates(endCropCoordinates)
      }
    })),
    useV2ModelConfig: true
  };
...
export function buildVideoReferenceImagesBody({
  ...
  return {
    ...
    requests: Array.from({ length: requestCount }, () => ({
      ...
      seed: seedFactory(),
      textInput: { structuredPrompt: { parts: [{ text: String(prompt || "").trim() }] } },
      videoModelKey: normalizeVideoModelKey("r2v", model, aspectRatio, { duration: videoLength }),
